...
pub fn main() {
    ...
    if project_path.exists() {
        ...
        println!("Creating Svelte project {}", input);
        let status = if cfg!(target_os = "windows") {
            Command::new("cmd")
                .args(&["/C", "npm", "create", "vite@latest", input, "--", "--template", "svelte"])
                .current_dir(&folder_path)
                .status()
        } else {
            Command::new("npm")
                .args(&["create", "vite@latest", input, "--", "--template", "svelte"])
                .current_dir(&folder_path)
                .status()
        };

        match status {
            Ok(s) if s.success() => println!("Successfully created Svelte project!"),
